...
#             print(self.human_phone_info(self.find(i)))
            if not self.find(i):
                continue
            phone, province, city, zip_code, area_code, phone_type = self.human_phone_info(self.find(i)).split('|')
            line = "%s|%s%s[%s]" % (phone, province, city,phone_type)
            all_phones.append(line)
        return all_phones
    
    def getAll(self):
        left = self.first_phone_record_offset
        right = self.phone_record_count
        all_phones = []
        while True:
            if left >= right:
                return
            current_offset = int(self.first_phone_record_offset + left * self.phone_fmt_length)
            buffer = self.buf[current_offset: current_offset + self.phone_fmt_length]
            cur_phone, record_offset, phone_type = struct.unpack(self.phone_fmt,
                                                                 buffer)
            record_content = get_record_content(self.buf, record_offset)
            province, city, zip_code, area_code = record_content.split('|')
            line = "%s|%s%s[%s]" % (cur_phone, province, city, Phone.get_phone_no_type(phone_type))
            all_phones.append(line)
            left += 1
        return all_phones
