/*
 * Author - Dax89
 * URL - https://github.com/QtGram/harbour-sailorgram/blob/0.9/harbour-sailorgram/qml/js/CountryList.js
 * 
 */
var countries = [
    { country: "阿尔巴尼亚", code:"355"},
    { country: "阿尔及利亚", code:"213"},
    { country: "安道尔", code:"376"},
    { country: "安哥拉", code:"244"},
    { country: "安提瓜和巴布达", code:"-267"},
    { country: "阿根廷", code:"54"},
    { country: "亚美尼亚", code:"374"},
    { country: "澳大利亚", code:"61"},
    { country: "奥地利", code:"43"},
    { country: "阿塞拜疆", code:"994"},
    { country: "巴哈马", code:"-241"},
    { country: "巴林", code:"973"},
    { country: "孟加拉国", code:"880"},
    { country: "巴巴多斯", code:"-245"},
    { country: "白俄罗斯", code:"375"},
    { country: "比利时", code:"32"},
    { country: "伯利兹", code:"501"},
    { country: "贝宁", code:"229"},
    { country: "不丹", code:"975"},
    { country: "玻利维亚", code:"591"},
    { country: "波斯尼亚和黑塞哥维那", code:"387"},
    { country: "博茨瓦纳", code:"267"},
    { country: "巴西", code:"55"},
    { country: "文莱", code:"673"},
    { country: "保加利亚", code:"359"},
    { country: "布基纳法索", code:"226"},
    { country: "布隆迪", code:"257"},
    { country: "柬埔寨", code:"855"},
    { country: "喀麦隆", code:"237"},
    { country: "加拿大", code:"1"},
    { country: "佛得角", code:"238"},
    { country: "中非共和国", code:"236"},
    { country: "乍得", code:"235"},
    { country: "智利", code:"56"},
    { country: "中华人民共和国", code:"86"},
    { country: "哥伦比亚", code:"57"},
    { country: "科摩罗", code:"269"},
    { country: "刚果(金沙萨)", code:"243"},
    { country: "刚果(布拉柴维尔)", code:"242"},
    { country: "哥斯达黎加", code:"506"},
    { country: "科特迪瓦(象牙海岸)", code:"225"},
    { country: "克罗地亚", code:"385"},
    { country: "古巴", code:"53"},
    { country: "塞浦路斯", code:"357"},
    { country: "捷克共和国", code:"420"},
    { country: "丹麦", code:"45"},
    { country: "吉布提", code:"253"},
    { country: "多米尼加", code:"-766"},
    { country: "多米尼加共和国(+1-809)", code:"+1-809"},
    { country: "多米尼加共和国(1-829)", code:"1-829"},
    { country: "厄瓜多尔", code:"593"},
    { country: "埃及", code:"20"},
    { country: "萨尔瓦多", code:"503"},
    { country: "赤道几内亚", code:"240"},
    { country: "厄立特里亚", code:"291"},
    { country: "爱沙尼亚", code:"372"},
    { country: "埃塞俄比亚", code:"251"},
    { country: "斐济", code:"679"},
    { country: "芬兰", code:"358"},
    { country: "法国", code:"33"},
    { country: "加蓬", code:"241"},
    { country: "冈比亚", code:"220"},
    { country: "佐治亚州", code:"995"},
    { country: "德国", code:"49"},
    { country: "加纳", code:"233"},
    { country: "希腊", code:"30"},
    { country: "格林纳达", code:"-472"},
    { country: "危地马拉", code:"502"},
    { country: "几内亚", code:"224"},
    { country: "几内亚比绍", code:"245"},
    { country: "圭亚那", code:"592"},
    { country: "海地", code:"509"},
    { country: "洪都拉斯", code:"504"},
    { country: "匈牙利", code:"36"},
    { country: "冰岛", code:"354"},
    { country: "印度", code:"91"},
    { country: "印度尼西亚", code:"62"},
    { country: "伊朗", code:"98"},
    { country: "伊拉克", code:"964"},
    { country: "爱尔兰", code:"353"},
    { country: "以色列", code:"972"},
    { country: "意大利", code:"39"},
    { country: "牙买加", code:"-875"},
    { country: "日本", code:"81"},
    { country: "约旦", code:"962"},
    { country: "哈萨克斯坦", code:"7"},
    { country: "肯尼亚", code:"254"},
    { country: "基里巴斯", code:"686"},
    { country: "朝鲜", code:"850"},
    { country: "韩国", code:"82"},
    { country: "科威特", code:"965"},
    { country: "吉尔吉斯斯坦", code:"996"},
    { country: "老挝", code:"856"},
    { country: "拉脱维亚", code:"371"},
    { country: "黎巴嫩", code:"961"},
    { country: "莱索托", code:"266"},
    { country: "利比里亚", code:"231"},
    { country: "利比亚", code:"218"},
    { country: "列支敦士登", code:"423"},
    { country: "立陶宛", code:"370"},
    { country: "卢森堡", code:"352"},
    { country: "马其顿", code:"389"},
    { country: "马达加斯加", code:"261"},
    { country: "马拉维", code:"265"},
    { country: "马来西亚", code:"60"},
    { country: "马尔代夫", code:"960"},
    { country: "马里", code:"223"},
    { country: "马耳他", code:"356"},
    { country: "马绍尔群岛", code:"692"},
    { country: "毛里塔尼亚", code:"222"},
    { country: "毛里求斯", code:"230"},
    { country: "墨西哥", code:"52"},
    { country: "密克罗尼西亚", code:"691"},
    { country: "摩尔多瓦", code:"373"},
    { country: "摩纳哥", code:"377"},
    { country: "蒙古", code:"976"},
    { country: "黑山代号", code: "382"},
    { country: "摩洛哥", code:"212"},
    { country: "莫桑比克", code:"258"},
    { country: "缅甸(缅甸)", code:"95"},
    { country: "纳米比亚", code:"264"},
    { country: "瑙鲁", code:"674"},
    { country: "尼泊尔", code:"977"},
    { country: "荷兰", code:"31"},
    { country: "新西兰", code:"64"},
    { country: "尼加拉瓜", code:"505"},
    { country: "尼日尔", code:"227"},
    { country: "尼日利亚", code:"234"},
    { country: "挪威", code:"47"},
    { country: "阿曼", code:"968"},
    { country: "巴基斯坦", code:"92"},
    { country: "帕劳", code:"680"},
    { country: "巴拿马", code:"507"},
    { country: "巴布亚新几内亚", code:"675"},
    { country: "巴拉圭", code:"595"},
    { country: "秘鲁", code:"51"},
    { country: "菲律宾", code:"63"},
    { country: "波兰", code:"48"},
    { country: "葡萄牙", code:"351"},
    { country: "卡塔尔", code:"974"},
    { country: "罗马尼亚", code:"40"},
    { country: "俄罗斯", code:"7"},
    { country: "卢旺达", code:"250"},
    { country: "圣基茨和尼维斯", code:"-868"},
    { country: "圣卢西亚", code:"-757"},
    { country: "圣文森特和格林纳丁斯", code:"-783"},
    { country: "萨摩亚", code:"685"},
    { country: "圣马力诺", code:"378"},
    { country: "圣多美和普林西比", code:"239"},
    { country: "沙特阿拉伯", code:"966"},
    { country: "塞内加尔", code:"221"},
    { country: "塞尔维亚", code:"381"},
    { country: "塞舌尔", code:"248"},
    { country: "塞拉利昂", code:"232"},
    { country: "新加坡", code:"65"},
    { country: "斯洛伐克", code:"421"},
    { country: "斯洛文尼亚", code:"386"},
    { country: "所罗门群岛", code:"677"},
    { country: "索马里", code:"252"},
    { country: "南非", code:"27"},
    { country: "西班牙", code:"34"},
    { country: "斯里兰卡", code:"94"},
    { country: "苏丹", code:"249"},
    { country: "苏里南", code:"597"},
    { country: "斯威士兰", code:"268"},
    { country: "瑞典", code:"46"},
    { country: "瑞士", code:"41"},
    { country: "叙利亚", code:"963"},
    { country: "塔吉克斯坦", code:"992"},
    { country: "坦桑尼亚", code:"255"},
    { country: "泰国", code:"66"},
    { country: "东帝汶(东帝汶)", code:"670"},
    { country: "多哥", code:"228"},
    { country: "汤加", code:"676"},
    { country: "特立尼达和多巴哥", code:"-867"},
    { country: "突尼斯", code:"216"},
    { country: "土耳其", code:"90"},
    { country: "土库曼斯坦", code:"993"},
    { country: "图瓦卢", code:"688"},
    { country: "乌干达", code:"256"},
    { country: "乌克兰", code:"380"},
    { country: "阿拉伯联合酋长国", code:"971"},
    { country: "英国", code:"44"},
    { country: "美国", code:"1"},
    { country: "乌拉圭", code:"598"},
    { country: "乌兹别克斯坦", code:"998"},
    { country: "瓦努阿图", code:"678"},
    { country: "梵蒂冈城", code:"379"},
    { country: "委内瑞拉", code:"58"},
    { country: "越南", code:"84"},
    { country: "也门", code:"967"},
    { country: "赞比亚", code:"260"},
    { country: "津巴布韦", code:"263"},
    { country: "阿布哈兹", code:"995"},
    { country: "中国", code:"886"},
    { country: "纳戈尔诺 - 卡拉巴赫", code:"277"},
    { country: "北塞浦路斯", code:"-302"},
    { country: "Pridnestrovie(德涅斯特河沿岸)", code:"-160"},
    { country: "索马里兰", code:"252"},
    { country: "南奥塞梯", code:"995"},
    { country: "圣诞岛", code:"61"},
    { country: "科科斯(基林)群岛", code:"61"},
    { country: "诺福克岛", code:"672"},
    { country: "新喀里多尼亚", code:"687"},
    { country: "法属波利尼西亚", code:"689"},
    { country: "马约特", code:"262"},
    { country: "圣巴泰勒米", code:"590"},
    { country: "圣马丁", code:"590"},
    { country: "圣皮埃尔和密克隆", code:"508"},
    { country: "瓦利斯和富图纳", code:"681"},
    { country: "库克群岛", code:"682"},
    { country: "纽埃", code:"683"},
    { country: "托克劳", code:"690"},
    { country: "根西岛", code:"44-1481"},
    { country: "马恩岛", code:"44-1624"},
    { country: "泽西岛", code:"44-1534"},
    { country: "安圭拉", code:"-263"},
    { country: "百慕大", code:"-440"},
    { country: "英属印度洋领地", code:"246"},
    { country: "英国主权基地区", code:"357"},
    { country: "英属维尔京群岛", code:"-283"},
    { country: "开曼群岛", code:"-344"},
    { country: "福克兰群岛(Islas Malvinas)", code:"500"},
    { country: "直布罗陀", code:"350"},
    { country: "蒙特塞拉特", code:"-663"},
    { country: "圣赫勒拿", code:"290"},
    { country: "特克斯和凯科斯群岛", code:"-648"},
    { country: "北马里亚纳群岛", code:"-669"},
    { country: "波多黎各(+1-787)", code:"+1-787"},
    { country: "波多黎各(1-939)", code:"1-939"},
    { country: "美属萨摩亚", code:"-683"},
    { country: "关岛", code:"-670"},
    { country: "美属维尔京群岛", code:"-339"},
    { country: "香港", code:"852"},
    { country: "澳门", code:"853"},
    { country: "法罗群岛", code:"298"},
    { country: "格陵兰", code:"299"},
    { country: "法属圭亚那", code:"594"},
    { country: "瓜德罗普岛", code:"590"},
    { country: "马提尼克岛", code:"596"},
    { country: "留尼旺", code:"262"},
    { country: "奥兰", code:"340"},
    { country: "阿鲁巴", code:"297"},
    { country: "荷属安的列斯群岛", code:"599"},
    { country: "斯瓦尔巴特", code:"47"},
    { country: "提升", code:"247"},
    { country: "特里斯坦达库尼亚", code:"290" }
    ]
